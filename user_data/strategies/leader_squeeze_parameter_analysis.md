...
因此，当前 `additional_entry_score=50` 配合最高 20% 热度惩罚时，原评分低于 62.5 的币不可能通过 50 分买入门槛。原评分 64 的币在满热度惩罚后仍有 51.2 分；原评分 61 的币会降到 48.8 分。
...
| additional_entry_score | 50.0 | 后续买入的评分门槛 |
...
快速通道的核心分上限 62 来自动量 32 分、放量 15 分和主动买 15 分；replacement_fast_core_score=48 表示至少取得其中 48 分。快速通道不要求旧仓原评分低于 45，但仍要求旧仓先通过 1h 走弱、回撤和最近 3 根 1h 高点保护条件；目标还必须通过 15m 突破、量比、主动买和收盘位置检查。普通目标实际仍须同时满足 additional_entry_score=50，因为轮换门槛取两者中的较高值。
