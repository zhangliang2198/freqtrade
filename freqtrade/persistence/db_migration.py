...
def migrate_db(session_target: SessionType):
    ...
    for row in Trade.session.scalars(select(ExchangeLedger)):
        ledger_count += 1
        make_transient(row)
        session_target.add(row)
    session_target.commit()
    ...
    max_ledger_id = session_target.scalar(select(func.max(ExchangeLedger.id)))
    ...
    set_sequence_ids(
        ...
        exchange_ledger_id=(max_ledger_id or 0) + 1,
    )

    logger.info(
        f"Migrated {trade_count} Trades, {pairlock_count} Pairlocks, "
        f"{kv_count} Key-Value pairs, {custom_data_count} Custom Data entries, "
        f"{wallet_history_count} Wallet History entries, "
        f"and {ledger_count} Exchange Ledger entries."
    )
