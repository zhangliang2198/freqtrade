...
class MD_SL(IStrategy):
    ...
    def bot_loop_start(self, current_time: datetime, **kwargs) -> None:
        ...
        (
            self.total_short_usdt,
            self.short_profit_ratio,
            self.total_long_usdt,
            self.long_profit_ratio,
            real_usdt,
            total_profit_pct,
        ) = self.get_assets_in_usdt()
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        # 只保留实际存在的列
        valid_cols = [
            col for col in [f"{pair}_1d_per" for pair in whitelist] if col in dataframe.columns
        ]
        current_col = f"{current_pair}_1d_per"
        if not valid_cols or current_col not in valid_cols:
            dataframe["momentum_rank"] = None
        else:
            dataframe["momentum_rank"] = dataframe[valid_cols].rank(axis=1, ascending=False)[
                current_col
            ]
    ...
    def get_assets_in_usdt(self):
        ...
        close_short_trades = [
            trade for trade in all_close_trades if trade.trade_direction == "short"
        ]
        ...
        logger.info(
            f"-----short账户利润百分比（持仓利润除以short实际余额）: {short_profit_ratio:.2f} %"
        )
        logger.info(
            f"-----short账户真实盈利百分比（所有short订单利润除以short初始资金）: {short_profit_real_ratio:.2f} %"
        )
        logger.info(f"--long")
        ...
        logger.info(
            f"-----long账户利润百分比（持仓利润除以long实际余额）: {long_profit_ratio:.2f} %"
        )
        logger.info(
            f"-----long账户真实盈利百分比（所有long订单利润除以long初始资金）: {long_profit_real_ratio:.2f} %"
        )
        logger.info(f"--总览")
        logger.info(f"-----当前账户实际余额(已包含持仓): {real_usdt:.2f} USDT")
        logger.info(f"-----当前持仓利润: {total_profit_amount:.2f} USDT")
        logger.info(
            f"-----当前账户真实盈利百分比（所有订单利润除以初始资金）: {total_profit_pct:.2f} %"
        )
