...
class Theshortgod(IStrategy):
    ...
    first_entry_ratio = 0.15

    # add_seq_X values define relative size for sequential DCA entries
    # Can be adjusted based on market conditions
    add_seq_1 = DecimalParameter(0.05, 0.2, default=0.1, decimals=2, space="buy", optimize=False)
    add_seq_2 = DecimalParameter(0.05, 0.2, default=0.15, decimals=2, space="buy", optimize=False)
    add_seq_3 = DecimalParameter(0.10, 0.30, default=0.3, decimals=2, space="buy", optimize=False)
    add_seq_4 = DecimalParameter(0.20, 0.50, default=0.5, decimals=2, space="buy", optimize=False)
    ...
    trailing_stop = (
        False  # Don't use built-in trailing, use custom_exit for "drawdown-based" profit taking
    )
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        used = float(
            trade.stake_amount
        )  # Used capital (based on stake, not notional+margin distinction)
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        if current_profit >= start:
            ...
            allow_draw = min(step * (1 + buckets), 1)
